...
export async function getCollaborativeRecommendations(userId: string, limit: number = 10): Promise<string[]> {
  ...
  try {
    ...
    const purchasedProducts = await Product.find({ _id: { $in: Array.from(purchasedProductIds) } }).lean();
    ...
    }).sort({ rating: -1, reviewCount: -1 }).limit(limit * 2).lean();
...
export async function getContentBasedRecommendations(productId: string, limit: number = 10): Promise<string[]> {
  ...
  try {
    ...
    }).sort({ rating: -1, reviewCount: -1 }).limit(limit).lean();
...
export async function getPersonalizedRecommendations(
  ...
  if (userId) productIds = await getCollaborativeRecommendations(userId, limit);
  if (productIds.length < limit && currentProductId) {
    const contentIds = await getContentBasedRecommendations(currentProductId, limit - productIds.length);
    const idSet = new Set(productIds);
    for (const id of contentIds) { if (!idSet.has(id)) productIds.push(id); }
  }
  if (productIds.length < limit) {
    const trendingIds = await getTrendingProducts(limit - productIds.length);
    const idSet = new Set(productIds);
    for (const id of trendingIds) { if (!idSet.has(id)) productIds.push(id); }
  }
  if (productIds.length === 0) {
    return Product.find({ isActive: true }).sort({ rating: -1, reviewCount: -1 }).limit(limit).lean();
  }
  const products = await Product.find({ _id: { $in: productIds }, isActive: true }).lean();
  const orderMap = new Map(products.map((p, i) => [p._id.toString(), i]));
  return productIds
    .map((id) => { const idx = orderMap.get(id); return idx !== undefined ? products[idx] : null; })
    .filter(Boolean)
    .slice(0, limit);
}
