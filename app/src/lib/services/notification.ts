...
function buildOrderEmailHtml(
  ...
  const itemRows = items
    ...
    .join("");
  const shippingText = shipping === 0 ? `<span style="color:#22c55e;font-weight:600;">FREE</span>` : formatPrice(shipping);
...
async function sendWhatsAppViaCloudAPI(phone: string, message: string): Promise<boolean> {
  ...
  try {
    ...
    if (res.ok) { console.log(`[Notification] WhatsApp Cloud sent to ${phone}`); return true; }
...
async function sendWhatsAppViaCallMeBot(phone: string, message: string): Promise<boolean> {
  ...
  try {
    ...
    const res = await fetch(`https://api.callmebot.com/whatsapp.php?phone=${phone}&text=${encoded}&apikey=${apiKey}`);
    if (res.ok) { console.log(`[Notification] CallMeBot WhatsApp sent to ${phone}`); return true; }
...
async function sendSMSViaMSG91(phone: string, message: string): Promise<boolean> {
  ...
  try {
    const res = await fetch("https://api.msg91.com/api/v5/flow", {
      ...
      body: JSON.stringify({ flow_id: process.env.MSG91_FLOW_ID, mobiles: `91${phone}`, VAR1: message }),
    });
    if (res.ok) { console.log(`[Notification] SMS sent to ${phone} via MSG91`); return true; }
...
async function sendGenericWebhook(orderId: string, message: string, customer: OrderCustomer, total: number): Promise<boolean> {
  ...
  try {
    ...
    if (res.ok) { console.log(`[Notification] Webhook sent for order ${orderId}`); return true; }
...
async function sendViaSmartFallback(
  ...
  const shortId = orderId.slice(-8).toUpperCase();
  const waUrl = `https://wa.me/${ADMIN_WHATSAPP}?text=${encodeURIComponent(`New Order #${shortId} from ${customer.name}\nTotal: ${formatPrice(total)}\n\nOpen: ${process.env.FRONTEND_URL || "https://dev-traders-two.vercel.app"}/dashboard/admin`)}`;
...
export async function sendOrderNotification(order: {
  ...
  const message = buildOrderMessage(orderId, order.items, order.customer, order.subtotal, order.shipping, order.total);
  const shortId = orderId.slice(-8).toUpperCase();
  ...
  switch (provider) {
    case "smart":
      sent = await sendViaSmartFallback(orderId, order.items, order.customer, order.subtotal, order.shipping, order.total, message);
      break;
    case "email":
  ...
  if (sent) console.log(`[Notification] ✅ Order #${shortId} notification sent`);
  else console.log(`[Notification] ⚠️  Order #${shortId} — no provider sent`);
}
