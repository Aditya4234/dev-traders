...
export async function GET(request: NextRequest) {
  try {
    ...
    const [
      ...
      Order.aggregate([{ $match: { createdAt: { $gte: startOfWeek } } }, { $group: { _id: null, total: { $sum: "$total" }, count: { $sum: 1 } } }]),
      Order.aggregate([{ $match: { createdAt: { $gte: startOfMonth } } }, { $group: { _id: null, total: { $sum: "$total" }, count: { $sum: 1 } } }]),
      Order.aggregate([{ $match: { createdAt: { $gte: new Date(now.getFullYear(), now.getMonth() - 1, 1), $lt: startOfMonth } } }, { $group: { _id: null, total: { $sum: "$total" }, count: { $sum: 1 } } }]),
      Order.countDocuments(),
      Order.countDocuments({ createdAt: { $gte: startOfDay } }),
      Order.countDocuments({ createdAt: { $gte: startOfWeek } }),
      Order.countDocuments({ createdAt: { $gte: startOfMonth } }),
      Order.countDocuments({ createdAt: { $gte: new Date(now.getFullYear(), now.getMonth() - 1, 1), $lt: startOfMonth } }),
      ...
      (async () => {
        ...
        for (let i = 6; i >= 0; i--) {
          ...
          daily.push({ date: dayStart.toISOString().slice(0, 10), sales: dayData[0]?.sales || 0, orders: dayData[0]?.orders || 0 });
        }
        return daily;
      })(),
    ]);
    ...
    const orderChange = ordersLastMonthCount > 0 ? Math.round(((ordersMonth - ordersLastMonthCount) / ordersLastMonthCount) * 100) : ordersMonth > 0 ? 100 : 0;
    ...
    for (let i = 6; i >= 0; i--) {
      ...
      monthlyChart.push({ month: monthNames[d.getMonth()], sales: mData[0]?.sales || 0, orders: mData[0]?.orders || 0 });
    }
    ...
    return NextResponse.json({
      ...
      dashboard: {
        ...
        orders: { total: totalOrders, today: ordersToday, thisWeek: ordersWeek, thisMonth: ordersMonth, orderChange, pending: pendingOrders, confirmed: confirmedOrders, shipped: shippedOrders, delivered: deliveredOrders, cancelled: cancelledOrders },
        payments: { cod: codOrders, online: onlineOrders, whatsapp: whatsappOrders },
        recentOrders,
        topProducts,
